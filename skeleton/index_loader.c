...
//map uniq_off to id
uint32_t *map_uniq_id = NULL;
uint32_t map_uniq_id_n = 0;
...
int load_index_file(char *index_dir){
    ...
    if(fp_num ==NULL){
        printf("File error opening the size of dm file:wrong dm route or wrong dm file name\n");
        exit(1);
    }
#ifdef SIZE64
    int temp;
    ...
    ref_seq_n = file_size;
    //printf("the reference seq ref_seq_n: %zu\n", ref_seq_n);
    ...
    */
    ...
    //uniq path total len:      result_seq * 32
    //uniq path id info in:     *buffer_seqf (result_seqf)
    map_uniq_id_n = result_seq * 32 / 16 + 1;
    map_uniq_id = (uint32_t *)malloc(sizeof(uint32_t) * map_uniq_id_n);
    uint32_t _idx = 0, _i = 0;
    for (_i=1; _i<result_seqf; ++_i)
    {
        uint32_t tar_id = buffer_seqf[_i];
        while (_idx * 16 < tar_id)
        {
            map_uniq_id[_idx] = _i-1;
            ++_idx;
        }
    }
    while (_idx * 16 <= result_seq * 32)
    {
        map_uniq_id[_idx] = _i-1;
        ++_idx;
    }
    ...
    //{
    //    fprintf(stderr, "[%zu %zu] ", k, buffer_seqf[k]);
    //}
    //fprintf(stderr, "\n");
    //for (uint32_t k=0; k<100; ++k)
    //{
    //    fprintf(stderr, "[%zu %zu] ", k, map_uniq_id[k]);
    //}
    //fprintf(stderr, "\n");
...
int chromosome_judge(uint64_t position)
{
	int file_n = 0;
