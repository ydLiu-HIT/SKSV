...
def statistics(call, ans, opt, res):
    for svtype in call:
        ...
        for ele in call[svtype]:
            if svtype == "BND" and opt == 4 and res == 2:
            ...
            # 	print(ele)
        logging.info('TP-%d of %s:\t%d\t%d\t%f' % (res, svtype, tp, total, tp/total))
    
    for svtype in ans:
        fn = 0
        total = 0
        for ele in ans[svtype]:
            if svtype == "TRA" and opt == 4:
                print("\t".join([str(i) for i in ele]))
            total += 1
            if svtype == 'INS':
                if ele[2+opt] >= res:
                ...
                if ele[3+opt] >= res:
                    fn += 1
        logging.info('TN-%d of %s:\t%d\t%d\t%f' % (res, svtype, fn, total, fn/total))
...
def load_ans(path):
    ...
    for line in file:
        ...
        if svtype in ['INS']:
            ansbed[svtype].append([chr, start, len(seq[4]), 0, 0, 0, 0, 0])
        elif svtype in ['BND']:
            chr2 = seq[4].split(':')[1]
            ...
            if strand1[0] == 'f':
                if strand2[0] == 'f':
                    ansbed[svtype].append(
                        [chr, start, chr2, start2, "N[[", 0, 0, 0, 0, 0])
                    # ansbed[svtype].append([chr, start, chr2, start2, "]]N", 0, 0, 0, 0])
                    # ansbed[svtype].append([chr, end, chr2, start2+end-start, "]]N", 0, 0, 0, 0])
                    ansbed[svtype].append(
                        [chr, end, chr2, start2+end-start, "N[[", 0, 0, 0, 0, 0])
                else:
                    ansbed[svtype].append(
                        [chr, start, chr2, start2, "N[[", 0, 0, 0, 0, 0])
                    ansbed[svtype].append(
                        [chr, start, chr2, start2+end-start, "[[N", 0, 0, 0, 0, 0])
                    ansbed[svtype].append(
                        [chr, end, chr2, start2, "N]]", 0, 0, 0, 0, 0])
                    ansbed[svtype].append(
                        [chr, end, chr2, start2+end-start, "]]N", 0, 0, 0, 0, 0])
            else:
                if strand2[0] == 'f':
            ...
            ansbed[svtype].append([chr, start, end, end-start+1, 0, 0, 0, 0, 0])
...
def main_ctrl(args):
    ...
    if args.choice == "BND":
        ...
        logging.info(
            "The number of calls within abnormal SV type in Sniffles:")
        ...
        sksv, absksv = load_callset(args.sksv, ["BND"])
        logging.info("The number of calls within abnormal SV type in SKSV:")
        for key in absksv:
            logging.info("<SKSV-%s>\t%d." % (key, absksv[key]))
        logging.info("Evaluation on SKSV callsets...")
        eval(sksv, ans, args.bias, args.offect, 5, genotype)
        statistics(sksv, ans, 5, 1)
        statistics(sksv, ans, 5, 2)
    ...
    if args.choice == "DUP":
        ...
        sksv, absksv = load_callset(args.sksv, ["INS", "DUP"])
        logging.info("The number of calls within abnormal SV type in SKSV:")
        for key in absksv:
            logging.info("<SKSV-%s>\t%d." % (key, absksv[key]))
        logging.info("Evaluation on SKSV callsets...")
    ...
    if args.choice == "IID":
        ...
        sniffles, absniffles = load_callset(
            args.sniffles, ["INS", "INV", "DEL"])
        logging.info(
            "The number of calls within abnormal SV type in Sniffles:")
        for key in absniffles:
            logging.info("<Sniffles-%s>\t%d." % (key, absniffles[key]))
        ...
        sksv, absksv = load_callset(args.sksv, ["INS", "INV", "DEL"])
        logging.info("The number of calls within abnormal SV type in SKSV:")
        for key in absksv:
            logging.info("<SKSV-%s>\t%d." % (key, absksv[key]))
        logging.info("Evaluation on SKSV callsets...")
...
def parseArgs(argv):
    parser = argparse.ArgumentParser(
        prog="Trio_eval", description=USAGE, formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument("choice", type=str,
                        help="Chose specific SV type.[IID/DUP/BND]")
    ...
    parser.add_argument('sksv', type=str, help="SKSV callsets")
    parser.add_argument(
        '-b', '--bias', help="Bias of overlaping.[%(default)s]", default=0.7, type=float)
    parser.add_argument(
        '-o', '--offect', help="Offect of translocation overlaping.[%(default)s]", default=1000, type=int)
