...
def generate_semi_inv_cluster(semi_inv_cluster, chr, svtype, read_count, sv_size,
                              candidate_single_SV, max_cluster_bias, svsegINFO, action, MaxSize, gt_round):
                ...
                if inv_len >= sv_size and max_count_id >= read_count:
                    ...
                    if inv_len <= MaxSize:
                        if action:
                            ...
                            DV, DR, GT, GL, GQ, QUAL = call_gt(svsegINFO, int(breakpoint_1),
                                                               int(breakpoint_2), chr, list(
                                                                   temp_id.keys()),
...
def call_gt(svsegINFO, pos_1, pos_2, chr, read_id_list, max_cluster_bias, gt_round):
    ...
    #print(chr, pos_1, search_start, search_end, up_bound, gt_round)
    #print(querydata1)
    #print(len(querydata1), len(read_id_list))

    search_start = max(int(pos_2 - max_cluster_bias/2), 0)
    search_end = int(pos_2 + max_cluster_bias/2)
    status2 = count_coverage(chr,
                            search_start,
                            search_end,
                            svsegINFO,
                            querydata2,
    ...
    #print(querydata2)
    #print(read_id_list)
    #print(len(querydata2), len(read_id_list))
    ...
    if status1 == -1:
        ...
        querydata = querydata1.intersection(querydata2)
