...
def rst_ctrl(args, argv):
    ...
    analysis_pools = Pool(processes=2)
...
def main_ctrl(args, argv):
    ...
    with open(fin_ref_fai, 'r') as fin:
        for row in fin:
            ...
            chr_name_list.append(contig_name)
    ...
    #for chr in ["1"]:
        for svtype in ["DEL", "INS", "INV"]:
            # if svtype == "INV":
            # 	para = [("%s%s.sigs"%(temporary_dir, svtype),
            # 				chr,
            # 				svtype,
            # 				args.min_support,
    ...
    generate_output(args, semi_result, contigINFO, argv)
